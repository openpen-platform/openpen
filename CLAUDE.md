...
- **MUST NOT** 在測試中使用 CSS class 選取器（`locator('.<class>')`、`querySelector('.<class>')`、`wrapper.find('.<class>')`）做互動或斷言定位——**MUST** 改用 `data-testid` 選取器（`getByTestId('...')` / `locator('[data-testid="..."]')`）。**唯一白名單**：`toHaveClass` / `not.toHaveClass` 是驗證 class 狀態的合法斷言，不是選取器，可保留。ESLint gate（`no-restricted-syntax`）在 `tests/` 下強制執行此規則。
