...
test('initial state: ball visible, control bar hidden', async () => {
  ...
  await expect(mainWindow.locator('.float-ball')).toBeVisible();
  await expect(mainWindow.locator('.control-bar')).not.toBeVisible();
});
...
test('clicking the ball expands the control bar', async () => {
  ...
  const ball = mainWindow.locator('.float-ball');
  await expect(ball).toBeVisible();

  await ball.click();

  await expect(mainWindow.locator('.control-bar')).toBeVisible();
  await expect(mainWindow.locator('.float-ball')).not.toBeVisible();
});

test('control bar shows the pin button', async () => {
  const mainWindow = await getMainWindow();
  // The previous test may leave the bar expanded; re-expand otherwise.
  const bar = mainWindow.locator('.control-bar');
  if (!(await bar.isVisible())) {
    await mainWindow.locator('.float-ball').click();
    await expect(bar).toBeVisible();
  }

  const pinBtn = mainWindow.locator('.cb-btn[data-tip="Pin"]');
